...
import json
import time
import asyncio#for making this asyncronous
...
'''
async def countdown(api_string):
...
async def limit_handled(cursor, api_string):
    while True:
        try:
            yield cursor.next()
...
#It then adds new retweets to the existing list in storage
'''
async def get_more_retweets(tweet):
    while True:
        ...
        try:
            ...
            print(f"and {len(existing_rts)} existing retweets")#debug
            ...
            print("Rate limted exceeded while gathering retweets for ", tweet)
            print(e)
            countdown(api_string)
...
async def build_network(tweet):
    ...
    while True:
        ...
        #tweet
    #Make sure this does not overwrite things done by other code during async wait times
    with open('storage.pkl', 'rb') as f:
        storage = pickle.load(f)
...
aync def update_tweet(tweet):
    get_more_retweets(tweet)
    build_network(tweet)
...
async def main(list_of_ids):
    ...
    while(True):
        ...
        for tweet in list_of_objects:
            ...
            print(f"Checking retweet count of {tweet.id}")#debug
            ...
            if new_count >= (old_count) or old_count == 0 :
                ...
                '''
                print(f"{tweet.id} has new retweets")
                #update_tweet(tweet)
                new_task = loop.create_task(update_tweet(tweet.id))
                task_list.append(new_task)
        if len(task_list) > 0:
            await asyncio.gather(task_list)

        time.sleep(5) #wait 1 second so that we can recheck retweet count a maximum of 900 times in 15 min
...
if __name__ == '__main__':
    ...
    list_of_ids = [tid, 1067605231817670657, 1064397964238573568, 1064292313797668869, 1064025163342200832, 1063959627761680386, 1071873259644354560, 1071870040901742592] #must not exceed 100 tweets

    #main(list_of_ids)
    loop = asyncio.get_event_loop()
    loop.set_debug(1)
    loop.run_until_complete(main(list_of_ids))
    loop.set_debug(1)
